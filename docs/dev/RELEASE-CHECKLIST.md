...
- [ ] `pnpm audit --prod --audit-level high` 通过；豁免只允许以 `pnpm-workspace.yaml` 的
      `auditConfig.ignoreGhsas` 显式登记，且每条豁免必须有对应 ADR。当前精确豁免为
      GHSA-f88m-g3jw-g9cj 与 GHSA-rgj7-g3m4-5g8c：sharp 固定 0.33.5（线上 CPU 只支持 x86-64-v1，≥ 0.34 的预编译包
      要求 x86-64-v2），由 `electron/services/sharpRuntime.cjs` 按 advisory 官方缓解禁用
      GIF/TIFF/VIPS/HEIF（含 AVIF）解码器；必须验证危险格式的 Buffer/文件输入拒绝、正常位图及 SVG 可用，候选部署也验证 GIF/AVIF 拒绝。理由与退出条件见
