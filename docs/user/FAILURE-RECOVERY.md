...
症状：探索/引导继承项目里，布局已批准过，但锁定风格后布局页提示
「画布或需求已变化」，布局与风格互相卡死。这是旧版缺陷把
`style_contract_regenerated` 错误传播到了布局链路；新版本不会再产生
新案例，存量项目可用一次性修复。

恢复：在布局页点「执行一次性修复」（`layout-repair`）。修复前会
完整备份原状态并重跑布局校验，只在失效原因确为旧版循环且输入未变
时恢复；不满足条件会抛 `ROUTE_CYCLE_REPAIR_INELIGIBLE`，此时按提示
