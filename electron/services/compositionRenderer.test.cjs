...
test('strict pipeline persists final output and final approval fails when its PNG is deleted', async () => {
  ...
  try {
    ...
    await projectStore.saveArtifact(project.id, 'font-manifest', { ...base, id: 'fonts', fonts: [{ id: 'ui', local_path: 'style/fonts/ui.ttf', file_hash: `sha256:${'c'.repeat(64)}`, license_status: 'confirmed', coverage: {} }], roles: {} });
