...
// 调整结果为准确答案，不再以此拦截。
// M4-I3：判定不读取模型自报的 coverage——模型不得通过自填 covered_items
// 自我声明已覆盖而不产出真实控件（与 recomputeCoverage 同一匹配语义）。
// M4-J1（审核 §6.3）：反馈按控件/信息分别输出，避免把信息类遗漏错误
// 引导到 required_controls，提高修复轮提示精度。
