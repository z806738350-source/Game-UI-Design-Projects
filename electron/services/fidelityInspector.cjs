...
async function inspectFidelityEvidence({ projectPath, project, manifest, output }) {
  ...
  try {
    ...
    if (!issues.some((item) => item.code === 'COMPOSITION_OUTPUT_UNREADABLE')) add('visual-fidelity', 'COMPOSITION_OUTPUT_UNREADABLE', error.message);
  }
  const canvas = { width: Number(manifest?.canvas?.[0]), height: Number(manifest?.canvas?.[1]) };
  if (metadata && (metadata.format !== 'png' || metadata.width !== canvas.width || metadata.height !== canvas.height)) add('visual-fidelity', 'FINAL_CANVAS_MISMATCH', 'Decoded final PNG does not match the Composition canvas.', { format: metadata.format, width: metadata.width, height: metadata.height, canvas });
  if (metadata && metadata.hasAlpha !== true) add('visual-fidelity', 'FINAL_ALPHA_MISSING', 'Final PNG must contain a real alpha channel.');
  if (stats?.channels?.[3]?.max === 0) add('visual-fidelity', 'FINAL_PIXELS_EMPTY', 'Final PNG alpha channel contains no visible pixels.');
  if (output?.version !== manifest?.version || output?.source?.composition_manifest_version !== manifest?.version) add('manifest-consistency', 'OUTPUT_VERSION_MISMATCH', 'Composition Output version does not match the current Manifest.');
  if (output?.source?.composition_manifest !== manifest?.id) add('manifest-consistency', 'OUTPUT_SOURCE_MISMATCH', 'Composition Output references a different Manifest.');
  ...
  for (const layer of manifest?.layers || []) {
    ...
    if (!rect) { add('visual-fidelity', 'LAYER_OUT_OF_BOUNDS', `${layer.control_id || layer.type} has an invalid or out-of-canvas rect.`); continue; }
    const diagnostic = diagnostics.find((item) => item.control_id === layer.control_id && (layer.type !== 'text' || item.font_role === layer.font_role));
    if (!diagnostic || !sameRect(rect, diagnostic.target_rect)) add('visual-fidelity', 'RENDERED_BBOX_MISMATCH', `${layer.control_id || layer.type} lacks matching rendered bbox evidence.`, { expected: rect, actual: diagnostic?.target_rect });
    let cropHash;
    ...
    if (slot?.safe_area_compliant === true) {
      ...
      if (rect.left < marginX || rect.top < marginY || rect.left + rect.width > canvas.width - marginX || rect.top + rect.height > canvas.height - marginY) add('visual-fidelity', 'SAFE_AREA_VIOLATION', `${layer.control_id} leaves the declared 5% safe area.`, { rect, margin_x: marginX, margin_y: marginY });
    }
    if (layer.type === 'component') {
      ...
      try {
        ...
        if (actual !== layer.asset_hash) add('manifest-consistency', 'COMPONENT_ASSET_HASH_MISMATCH', `${layer.component_id} current file hash differs from the Manifest.`);
      } catch (error) { add('manifest-consistency', 'COMPONENT_ASSET_UNREADABLE', `${layer.component_id}: ${error.message}`); }
      if (layer.renderer === 'nine-slice') {
        const fixed = diagnostic?.patches?.filter((patch) => patch.fixed_corner) || [];
        if (fixed.length !== 4) add('visual-fidelity', 'NINE_SLICE_FIXED_REGIONS_MISSING', `${layer.component_id} does not record four fixed corners.`);
        for (const patch of fixed) if (patch.source.width !== patch.destination.width || patch.source.height !== patch.destination.height || patch.source_hash !== patch.rendered_hash) add('visual-fidelity', 'NINE_SLICE_FIXED_REGION_DEFORMED', `${layer.component_id} changed a fixed corner.`, patch);
      }
      ...
      try {
        ...
        if (inspected.file_hash !== layer.font_hash) add('manifest-consistency', 'FONT_ASSET_HASH_MISMATCH', `${layer.font_id} current file hash differs from the Manifest.`);
        if (inspected.family_name !== layer.font_family || inspected.postscript_name !== layer.postscript_name) add('manifest-consistency', 'FONT_IDENTITY_MISMATCH', `${layer.font_id} current identity differs from the Manifest.`);
      } catch (error) { add('manifest-consistency', 'FONT_ASSET_UNREADABLE', `${layer.font_id}: ${error.message}`); }
      if (!diagnostic?.ink_bounds || diagnostic.text_overflow || diagnostic.ink_bounds.touches_boundary) add('visual-fidelity', 'TEXT_OVERFLOW', `${layer.control_id} text touches or exceeds its rendered slot.`, { ink_bounds: diagnostic?.ink_bounds, target_rect: rect });
    }
  }
  for (const family of project.artifacts?.componentContract?.families || []) for (const state of Object.values(family.states || {})) {
    ...
    try {
      ...
      if (actual !== state.asset_hash) add('manifest-consistency', 'COMPONENT_ASSET_HASH_MISMATCH', `${family.id} current file hash differs from the Component Contract.`);
    } catch (error) { add('manifest-consistency', 'COMPONENT_ASSET_UNREADABLE', `${family.id}: ${error.message}`); }
  }
  for (const font of project.artifacts?.fontManifest?.fonts || []) {
    ...
    try {
      ...
      if (inspected.file_hash !== font.file_hash) add('manifest-consistency', 'FONT_ASSET_HASH_MISMATCH', `${font.id} current file hash differs from the Font Manifest.`);
      if (inspected.family_name !== font.family_name || inspected.postscript_name !== font.postscript_name) add('manifest-consistency', 'FONT_IDENTITY_MISMATCH', `${font.id} current identity differs from the Font Manifest.`);
    } catch (error) { add('manifest-consistency', 'FONT_ASSET_UNREADABLE', `${font.id}: ${error.message}`); }
  }
  for (let first = 0; first < componentRects.length; first += 1) for (let second = first + 1; second < componentRects.length; second += 1) {
    ...
    if (!left.allow_overlap && !right.allow_overlap && intersectionArea(left.rect, right.rect) > 0) add('visual-fidelity', 'COMPONENT_OVERLAP', `${left.control_id} overlaps ${right.control_id}.`, { left: left.rect, right: right.rect });
  }
  const evidence = {
