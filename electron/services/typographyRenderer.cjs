...
async function renderTextLayer({ projectPath, layer, target, resolveProjectPath, hashBuffer }) {
  ...
  try {
    ...
    if (inspected.file_hash !== layer.font_hash) throw Object.assign(new Error('Font asset hash changed.'), { code: 'FONT_ASSET_HASH_MISMATCH' });
  ...
  if (final) throw Object.assign(new Error(`Exact font failed to load for ${layer.font_role}: ${error.message}`), { code: 'FONT_ACTUAL_LOAD_FAILED', cause_code: error.code, cause: error });
