...
async function executeRepairTask({ task, contract, critique, capabilities, providerClient, providerConfig, sourcePath, overlayPath, componentBoardPath, maskPath, size, prompt }) {
  if (task.repair_mode === 'inpaint' && (!capabilities.supports_inpaint || !maskPath)) throw Object.assign(new Error('Inpaint capability or repair mask is unavailable.'), { code: 'INPAINT_NOT_AVAILABLE' });
  if (!sourcePath || !overlayPath || !componentBoardPath) throw Object.assign(new Error('Repair requires parent Underlay, Review Overlay, and component board files.'), { code: 'REPAIR_EVIDENCE_INCOMPLETE' });
  ...
  if (!imageUrl) throw Object.assign(new Error('Repair provider returned no image.'), { code: 'REPAIR_OUTPUT_MISSING' });
