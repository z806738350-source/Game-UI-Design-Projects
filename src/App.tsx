...
export function App() {
  ...
  return <div className={`app-shell ${project ? 'has-project' : ''} ${inspectorOpen || assistantOpen ? 'has-inspector' : ''} ${assistantOpen ? 'has-assistant' : ''} ${config?.platform === 'darwin' ? 'is-mac' : ''} ${galleryOpen ? 'is-gallery' : ''}`}>
    ...
    {config?.features.assistant && <AssistantPanel activeStage={activeStage} project={project} open={assistantOpen} inert={galleryOpen} onProjectRefresh={refreshAssistantProject} onClose={() => { setAssistantOpen(false); assistantEntryRef.current?.focus(); }} />}
