...
import type { AppConfig, Artifact, CreateProjectInput, DesignProject, LayoutProposal, ProjectSummary, ScreenControl, VisualVariation } from './types';
import { StrictContinuationPanel } from './features/strict-continuation/StrictContinuationPanel';
import { StrictProductionPanel } from './features/strict-continuation/StrictProductionPanel';
import { ReferenceWorkbench } from './features/workbenches/ReferenceWorkbench';
...
const screenInput = (project: DesignProject, input: Record<string, unknown> = {}) => ({ screenId: project.screen_id, ...input });
...
function InputWorkspace({ project, busy, run }: WorkspaceProps) {
  ...
  <WireframeReference project={project} editable busy={busy} onOpen={() => setWireframeOpen(true)} onReplace={() => run(() => copilotApi.importFile(project.id, 'wireframe', project.screen_id), { label: '导入 UE 线框稿' })} />
    ...
    <div className="workspace-footer"><span className={ready ? 'ready-copy' : ''}>{!ready ? '请先导入 UE 线框稿。' : !hasIntent ? 'UE 已就绪，先让 AI 生成一份可确认的设计意图。' : confirmed ? '设计意图已确认，可以进入功能解读。' : '请检查并确认设计意图，再生成完整功能契约。'}</span>{!hasIntent ? <button className="button button--primary" disabled={busy || !ready} onClick={() => run(async () => { await copilotApi.saveProject(project.id, { artDirection, screenId: project.screen_id } as never); return copilotApi.draftRequirement(project.id, project.screen_id); }, { label: 'AI 正在读取 UE 并预填意图' })}><ScanSearch size={17} />AI 解读并预填写</button> : <button className="button button--primary" disabled={busy || !ready} onClick={() => run(async () => { await saveInput(true); return copilotApi.runStage(project.id, 'wireframe_interpretation', screenInput(project, { stayOnInputUntilComplete: true })); }, { label: '正在生成完整功能契约' })}><Bot size={17} />{confirmed ? '重新生成功能契约' : '确认意图并开始功能解读'}</button>}</div>
...
function ContractWorkspace({ project, busy, run }: WorkspaceProps) {
  ...
  const makeDraft = (): ContractDraft => ({
    ...
    required_controls: Array.isArray(artifact?.required_controls) ? (artifact.required_controls as unknown[]).map((control, index) => typeof control === 'string'
      ? { id: `control-${index + 1}`, label: control, role: 'action', required: true, migrated_from_label: control }
      : { id: String((control as ScreenControl).id), label: String((control as ScreenControl).label), role: String((control as ScreenControl).role || 'action'), required: (control as ScreenControl).required !== false, ...((control as ScreenControl).migrated_from_label ? { migrated_from_label: (control as ScreenControl).migrated_from_label } : {}) }) : [],
    ...
    setDraft((current) => {
      if (editingItem.key === 'required_controls') {
        const control = {
          ...
          role: String(editingItem.role || 'action').trim(), required: editingItem.required !== false
        };
