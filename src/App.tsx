...
import {
  ...
  Download, Edit3, Eye, FileJson, FolderOpen, ImagePlus, Layers3, LoaderCircle,
...
function NewProjectDialog({ onCreate, onClose, busy }: { onCreate: (input: CreateProjectInput) => void; onClose?: () => void; busy: boolean }) {
  ...
  return <div className="dialog-backdrop">
    <form className="create-dialog" onSubmit={(event) => { event.preventDefault(); onCreate(draft); }}>
      ...
      <div className="dialog-actions">{onClose && <button type="button" className="button button--ghost" onClick={onClose}>取消</button>}<button disabled={busy || !draft.name.trim()} className="button button--primary" type="submit">{busy ? <LoaderCircle className="spin" size={16} /> : <ArrowRight size={16} />}创建并进入工作台</button></div>
    </form>
  </div>;
}
...
function InputWorkspace({ project, busy, run }: WorkspaceProps) {
  ...
  return <div className="workspace-content input-workspace">
...
function VisualWorkspace({ project, busy, run, canCancel, onCancel }: WorkspaceProps & { canCancel: boolean; onCancel: () => void }) {
  ...
  return <div className="workspace-content visual-workspace">
...
export function App() {
  ...
  return <div className={`app-shell ${inspectorOpen ? 'has-inspector' : ''} ${config?.platform === 'darwin' ? 'is-mac' : ''}`}>
    ...
    {inspectorOpen && <aside className="artifact-inspector"><div className="inspector-head"><span>AI 输入、产物与版本</span><FileJson size={17} /></div>{activeStage === 'input' && project ? <InputSourceSummary project={project} /> : currentArtifact ? <JsonSummary artifact={currentArtifact} history={project?.artifactHistory?.filter((item) => item.id === currentArtifact.id || item.kind.includes(activeStage.split('_')[0]))} /> : <EmptyArtifact title="当前阶段尚无 AI 产物" copy="完成当前步骤后，版本、来源、结构化内容和历史快照会显示在这里。" />}<div className="inspector-foot"><b>本地项目目录</b><code>{config?.workspaceRoot || 'Loading…'}</code><small>批准与生成结果均保存在这里，可随时回看历史版本。</small></div></aside>}
    {createOpen && <NewProjectDialog onCreate={create} onClose={projects.length ? () => setCreateOpen(false) : undefined} busy={busy} />}{settingsOpen && config && <SettingsDialog config={config} onSaved={setConfig} onClose={() => setSettingsOpen(false)} />}{managerOpen && project && <ProjectManager project={project} projects={projects} busy={busy} run={run} onClose={() => setManagerOpen(false)} onSwitch={switchProject} />}
  </div>;
}
