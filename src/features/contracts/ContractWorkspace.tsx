...
import { CONTROL_ROLE_OPTIONS, CONTROL_ROLE_LABELS } from '../binding/BindingWorkbench';
...
export function ContractWorkspace({ project, busy, run, onNavigate }: WorkspaceProps & { onNavigate: (stage: StageId) => void }) {
  ...
  // 语义角色解析是 strict 路线绑定策略的前置要求（后端在绑定批准时以
  // BINDING_GENERIC_ROLE_UNRESOLVED 拒绝未解析的通用角色）；探索/引导路线
  // 不生产组件绑定，角色保持可选——未选具体角色时回落通用 'action'，
  // 列表显示「待语义解析」，不阻断设计师保存条目。
  const roleResolutionRequired = profile === 'strict';
  ...
  const commitItem = () => {
    ...
    setDraft((current) => {
      if (editingItem.key === 'required_controls') {
        const chosenRole = String(editingItem.role || '').trim();
        const control = {
          id: String(editingItem.controlId || '').trim(), label: value,
          // 角色值始终取自受控词表；非 strict 路线未选具体角色时回落通用
          // 'action'（列表显示「待语义解析」），strict 路线的门禁见下。
          role: CONTROL_ROLE_OPTIONS.includes(chosenRole) ? chosenRole : 'action',
          required: editingItem.required !== false
        };
        if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(control.id)) return current;
        // 仅 strict 路线强制解析语义角色（绑定策略前置）；探索/引导路线
        // 不生产绑定，不得因角色未选阻断保存。
        if (roleResolutionRequired && !CONTROL_ROLE_OPTIONS.includes(control.role)) return current;
  ...
  return <>
    ...
    {artifact && workbenchOpen && <div className="contract-focus-backdrop"><section className={`contract-focus-workbench ${summaryOpen ? 'has-summary' : ''}`}><header className="focus-header"><div><span>01 · CONTRACT REVIEW</span><h2>功能契约专注检查</h2><p>所有修改先保存在本轮草稿中，点击“保存本轮修改”后统一生成一个版本。</p></div><div><span className={dirty ? 'draft-state is-dirty' : 'draft-state'}>{dirty ? '有未保存修改' : '当前内容已保存'}</span><button className="icon-button" onClick={requestCloseWorkbench} aria-label="关闭专注检查"><X size={20} /></button></div></header>
      ...
      <div className="focus-body"><main className="focus-list-panel"><header className="focus-category-heading"><div><span>{activeDefinition.eyebrow}</span><h3>{activeDefinition.label}<em>{activeItems.length} 项</em></h3><p>{activeDefinition.description}</p></div><button className="button button--secondary" data-testid="contract-edit-summary" onClick={() => setSummaryOpen((current) => !current)}><Edit3 size={15} />{summaryOpen ? '收起页面摘要' : '编辑页面摘要'}</button></header>
        <div className="focus-tools"><label className="category-search"><Search size={16} /><input value={categoryQuery} onChange={(event) => setCategoryQuery(event.target.value)} placeholder={`搜索全部${activeDefinition.label}`} /></label><div className="status-filters">{([['all', '全部'], ['unreviewed', '未检查'], ['confirmed', '已确认'], ['changed', '已调整'], ['question', '有疑问']] as const).map(([value, label]) => <button key={value} className={statusFilter === value ? 'is-active' : ''} onClick={() => setStatusFilter(value)}>{label}</button>)}</div></div>
        <div className="focus-items">{filteredActiveItems.length ? filteredActiveItems.map(({ item, index, status }) => editingItem?.key === activeCategory && editingItem.index === index ? <div className="focus-item is-editing" key={`${activeCategory}-${index}`}>{activeCategory === 'required_controls' && <div className="control-identity-fields"><label><span>稳定 ID</span><input value={editingItem.controlId || ''} onChange={(event) => setEditingItem({ ...editingItem, controlId: event.target.value })} /></label><label><span>语义角色</span><Dropdown ariaLabel="选择控件语义角色" value={CONTROL_ROLE_OPTIONS.includes(editingItem.role || '') ? editingItem.role || '' : ''} onChange={(next) => setEditingItem({ ...editingItem, role: next })} placeholder={roleResolutionRequired ? '选择具体角色（不得为 action）' : '可选：不选则保持通用角色'} options={CONTROL_ROLE_OPTIONS.map((role) => ({ value: role, label: CONTROL_ROLE_LABELS[role] || role }))} /></label><label><input type="checkbox" checked={editingItem.required !== false} onChange={(event) => setEditingItem({ ...editingItem, required: event.target.checked })} />必需控件</label></div>}<textarea autoFocus value={editingItem.value} onChange={(event) => setEditingItem({ ...editingItem, value: event.target.value })} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === 'Enter') commitItem(); if (event.key === 'Escape') setEditingItem(null); }} /><div><small>⌘ Enter 保存</small><button className="button button--primary" onClick={commitItem} disabled={!editingItem.value.trim() || (activeCategory === 'required_controls' && (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(editingItem.controlId || '') || (roleResolutionRequired && !CONTROL_ROLE_OPTIONS.includes(editingItem.role || ''))))}><Save size={15} />保存条目</button><button className="button button--ghost" onClick={() => setEditingItem(null)}>取消</button></div></div> : <article className={`focus-item status-${status}`} key={`${activeCategory}-${index}-${itemLabel(item)}`}><span>{String(index + 1).padStart(2, '0')}</span><p>{itemLabel(item)}{typeof item !== 'string' && <small className="control-id-copy">{item.id} · {CONTROL_ROLE_LABELS[item.role] || item.role} · {item.required ? '必需' : '可选'}</small>}</p><div className="item-status"><em>{statusCopy[status]}</em><button className={status === 'confirmed' ? 'is-active' : ''} title="标记为已确认" onClick={() => setItemStatus(activeCategory, index, 'confirmed')}><Check size={15} /></button><button className={status === 'question' ? 'is-active is-question' : ''} title="标记为有疑问" onClick={() => setItemStatus(activeCategory, index, 'question')}><MessageSquare size={15} /></button><button title="编辑条目" onClick={() => setEditingItem({ key: activeCategory, index, value: itemLabel(item), ...(typeof item === 'string' ? {} : { controlId: item.id, role: item.role, required: item.required }) })}><Edit3 size={15} /></button><button className="is-delete" title="删除条目" onClick={() => removeItem(activeCategory, index)}><Trash2 size={15} /></button></div></article>) : <div className="contract-items-empty">没有匹配条目，可以切换筛选条件或新增条目。</div>}</div>
        <div className="focus-add">{editingItem?.key === activeCategory && editingItem.index < 0 ? <div className="focus-item is-editing is-new">{activeCategory === 'required_controls' && <div className="control-identity-fields"><label><span>稳定 ID</span><input autoFocus placeholder="例如 confirm-purchase" value={editingItem.controlId || ''} onChange={(event) => setEditingItem({ ...editingItem, controlId: event.target.value })} /></label><label><span>语义角色</span><Dropdown ariaLabel="选择控件语义角色" value={CONTROL_ROLE_OPTIONS.includes(editingItem.role || '') ? editingItem.role || '' : ''} onChange={(next) => setEditingItem({ ...editingItem, role: next })} placeholder={roleResolutionRequired ? '选择具体角色（不得为 action）' : '可选：不选则保持通用角色'} options={CONTROL_ROLE_OPTIONS.map((role) => ({ value: role, label: CONTROL_ROLE_LABELS[role] || role }))} /></label><label><input type="checkbox" checked={editingItem.required !== false} onChange={(event) => setEditingItem({ ...editingItem, required: event.target.checked })} />必需控件</label></div>}<textarea autoFocus={activeCategory !== 'required_controls'} placeholder={`输入新的${activeDefinition.label}标签`} value={editingItem.value} onChange={(event) => setEditingItem({ ...editingItem, value: event.target.value })} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === 'Enter') commitItem(); if (event.key === 'Escape') setEditingItem(null); }} /><div><button className="button button--primary" onClick={commitItem} disabled={!editingItem.value.trim() || (activeCategory === 'required_controls' && (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(editingItem.controlId || '') || (roleResolutionRequired && !CONTROL_ROLE_OPTIONS.includes(editingItem.role || ''))))}><Save size={15} />加入草稿</button><button className="button button--ghost" onClick={() => setEditingItem(null)}>取消</button></div></div> : <button className="add-contract-item" onClick={() => setEditingItem({ key: activeCategory, index: -1, value: '', ...(activeCategory === 'required_controls' ? { controlId: `control-${draft.required_controls.length + 1}`, role: '', required: true } : {}) })}><Plus size={18} />新增{activeDefinition.label}条目</button>}</div>
      </main><aside className="focus-reference"><WireframeReference project={project} onOpen={() => setWireframeOpen(true)} /></aside></div>
      <footer className="focus-footer"><div><b>{filteredActiveItems.length} 项当前可见</b><span>连续滚动浏览，不再分页</span></div><div><button className="button button--ghost" onClick={requestCloseWorkbench}>关闭</button><button className="button button--primary" data-testid="contract-save-workbench" disabled={busy || !dirty || !draft.screen_name.trim() || !draft.purpose.trim() || !draft.primary_action.trim()} onClick={saveWorkbench}><Save size={16} />{dirty ? '保存本轮修改' : '暂无待保存更改'}</button></div></footer>
