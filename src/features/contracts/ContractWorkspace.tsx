  ...
  const commitItem = () => {
    ...
    setDraft((current) => {
      if (editingItem.key === 'required_controls') {
        const role = String(editingItem.role || '').trim();
        const control = {
          id: String(editingItem.controlId || '').trim(), label: value,
          // Roles use the controlled binding-policy vocabulary; the generic
          // legacy 'action' role can never be (re)assigned to a control, and
          // there is no implicit fallback: the commit gate stays disabled
          // until the designer picks a concrete role explicitly.
          role,
          required: editingItem.required !== false
        };
        if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(control.id)) return current;
        if (!CONTROL_ROLE_OPTIONS.includes(control.role)) return current;
