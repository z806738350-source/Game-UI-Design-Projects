...
export function InputWorkspace({ project, busy, run }: WorkspaceProps) {
  ...
  return <>
    ...
    <div className="input-grid input-grid--reworked">
      ...
      {project.project_type === 'existing' && <label className="field-card continuation-mode-card"><span><div><b>继承强度</b><em>全局输入</em></div><small>切换会按依赖图使模式相关的下游批准失效；锁定继承的项目不可切换</small></span><select data-testid="continuation-mode-select" disabled={busy || project.continuation_mode === 'locked-continuation'} value={project.continuation_mode === 'locked-continuation' ? 'existing-strict' : project.continuation_mode} onChange={(event) => run(() => copilotApi.saveProject(project.id, { continuationMode: event.target.value, screenId: project.screen_id } as never), { label: '切换继承强度' })}><option value="existing-strict">严格继承（推荐）</option><option value="existing-guided">引导继承</option></select></label>}
      </div>
