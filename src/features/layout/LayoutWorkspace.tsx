...
export function LayoutWorkspace({ project, busy, run, onNavigate }: WorkspaceProps & { onNavigate: (stage: StageId) => void }) {
  ...
  useEffect(() => { setSelected(preferredProposalId); setNotes(approvedNotes); setApproveError(''); }, [project.id, project.artifacts.layouts?.version, preferredProposalId, approvedNotes]);
