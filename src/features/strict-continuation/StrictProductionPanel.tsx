import { Check, RefreshCw, ScanSearch, ShieldCheck, WandSparkles } from 'lucide-react';
...
export function StrictProductionPanel({ project, underlayId, busy, run }: { project: DesignProject; underlayId?: string; busy: boolean; run: Run }) {
  ...
  return <section className="strict-production">
    <header><div><span>STRICT PRODUCTION</span><h3>Underlay → Critique → Composition → Fidelity</h3></div><div>{[['Critique', critiquePassed], ['Composition', Boolean(composition)], ['Fidelity', fidelity?.status === 'passed']].map(([label, ready]) => <i className={ready ? 'is-ready' : ''} key={String(label)}>{ready && <Check size={12} />}{label}</i>)}</div></header>
    ...
    <nav>
      ...
      <button className="button button--secondary" disabled={busy || !critiquePassed || !underlayId} onClick={() => run(() => copilotApi.composeVisual(project.id, { variationId: underlayId, mode: 'final' }), { label: '生成 Final Composition Manifest', stage: 'visual_exploration' })}><ShieldCheck size={15} />Final 合成</button>
      <button className="button button--secondary" disabled={busy || composition?.mode !== 'final'} onClick={() => run(() => copilotApi.runFidelity(project.id), { label: '运行 Final Fidelity Gate', stage: 'visual_exploration' })}><ShieldCheck size={15} />Fidelity 检查</button>
      <button className="button button--primary" disabled={busy || fidelity?.status !== 'passed' || composition?.mode !== 'final'} onClick={() => run(() => copilotApi.approveArtifact(project.id, 'composition-manifest'), { label: '批准最终严格继承结果', stage: 'visual_exploration' })}><Check size={15} />最终批准</button>
    </nav>
  </section>;
}
