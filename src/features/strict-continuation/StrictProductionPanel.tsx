...
export function StrictProductionPanel({ project, underlayId, busy, run }: { project: DesignProject; underlayId?: string; busy: boolean; run: Run }) {
  ...
  return <section className="strict-production">
    <header><div><span>STRICT PRODUCTION</span><h3>结构底层 → 污染审查 → 合成输出 → 保真校验</h3>{output && <small>{String(output.path)} · {String(output.width)}×{String(output.height)} · {String(output.hash).slice(0, 20)}…</small>}</div><div>{([['strict-gate-critique', '污染审查', critiquePassed], ['strict-gate-final-png', '最终 PNG', output?.mode === 'final'], ['strict-gate-fidelity', '保真校验', fidelity?.status === 'passed']] as Array<[string, string, boolean]>).map(([testId, label, ready]) => <i data-testid={testId} className={ready ? 'is-ready' : ''} key={testId}>{ready && <Check size={12} />}{label}</i>)}</div></header>
    ...
    <nav>
      ...
      <button className="button button--secondary" data-testid="composition-final" disabled={busy || !critiquePassed || !underlayId} onClick={composeFinal}><ShieldCheck size={15} />加载字体并生成最终 PNG</button>
      ...
      <button className="button button--ghost" data-testid="final-export" disabled={busy || output?.mode !== 'final'} onClick={() => run(async () => { await copilotApi.exportVisual(project.id, underlayId || 'final'); return project; }, { label: '导出最终 PNG', stage: 'visual_exploration' })}><Download size={15} />导出最终 PNG</button>
