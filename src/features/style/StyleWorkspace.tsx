...
// dedicated workbenches. 风格分析只由本页 style-generate 按钮显式触发；
// 从其他阶段导航进来不会自动调用模型。
export function StyleWorkspace({ project, busy, run }: WorkspaceProps) {
  ...
  // 已有项目只要求逐张批准至少一张参考图：Pack 只收录 approved 附件，
  // Inventory 清单状态不参与生成门禁，故不再作为前置条件。
  const canGenerate = (strict ? project.artifacts.screenContract?.status === 'approved' : project.artifacts.approvedLayout?.status === 'approved') && (project.project_type === 'new' || references.some((asset) => asset.approved));
  ...
  const stylePack = project.artifacts.referencePack;
  const styleOmission = stylePack?.purpose === 'style-resolution' && stylePack?.requires_omission_confirmation === true;
