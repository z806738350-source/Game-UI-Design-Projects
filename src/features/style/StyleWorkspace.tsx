...
export function StyleWorkspace({ project, busy, run }: WorkspaceProps) {
  ...
  return <>
    ...
    <div className="workspace-footer"><button className="button button--ghost" data-testid="style-generate" disabled={busy || !canGenerate} onClick={() => run(() => copilotApi.runStage(project.id, 'style_resolution', { confirmReferenceOmissions: project.artifacts.referencePack?.requires_omission_confirmation === true }), { label: project.artifacts.referencePack?.requires_omission_confirmation ? '确认省略项并解析风格' : artifact?.status === 'stale' ? '根据新参考重新解析风格' : '解析视觉风格', stage: 'style_resolution' })}><RefreshCw size={15} />{project.artifacts.referencePack?.requires_omission_confirmation ? '确认省略项并继续' : artifact?.status === 'stale' ? '参考已变化，重新解析风格' : artifact ? '重新解析风格' : '开始风格分析'}</button>{!canGenerate && <span className="stale-guidance">{(strict ? project.artifacts.screenContract?.status : project.artifacts.approvedLayout?.status) !== 'approved' ? (strict ? '请先批准功能契约。' : '请先批准布局。') : '已有项目必须先批准至少一张参考图及 Inventory。'}</span>}{artifact && artifact.status !== 'approved' && artifact.status !== 'stale' && <button className="button button--primary" data-testid="style-approve" disabled={busy} onClick={() => run(() => copilotApi.approveArtifact(project.id, 'style-contract'), { label: '批准并锁定风格', stage: 'style_resolution' })}><LockKeyhole size={16} />批准并锁定</button>}{artifact?.status === 'approved' && !strict && <button className="button button--primary" disabled={busy} onClick={() => run(() => copilotApi.runStage(project.id, 'visual_exploration'), { label: '生成 3 个视觉方向', stage: 'visual_exploration', total: 3 })}><WandSparkles size={16} />生成 3 个方向</button>}{artifact?.status === 'approved' && strict && <span className="stale-guidance">严格继承：先完成字体、组件与绑定，再生成组件感知布局。</span>}</div>
    </>;
}
