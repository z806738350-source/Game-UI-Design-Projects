...
export function ReferenceWorkbench({ project, busy, run }: { project: DesignProject; busy: boolean; run: Run }) {
  ...
  return <section className="reference-workbench">
    <header><div><span>REFERENCE INVENTORY / PACK</span><h3>参考图清单与实际附件顺序</h3><p>逐张声明用途并批准；生成服务只会收到 Pack 中按序列出的附件。</p></div><button className="button button--secondary" disabled={busy} onClick={() => run(() => copilotApi.importFile(project.id, 'reference'), { label: '添加风格参考' })}><ImagePlus size={15} />批量添加</button></header>
    ...
    {pack && <div className="reference-pack-preview"><header><b>Provider 容量 {String((pack.capacity_decision as Record<string, unknown>)?.used || 0)} / {String(pack.provider_limit || 0)}</b><span>{pack.status === 'approved' ? '附件顺序已确认' : '等待确认省略项'}</span></header><ol>{((pack.attachment_order as Array<Record<string, unknown>>) || []).map((item) => <li key={String(item.id)}><b>附件 {String(item.index)}</b><span>{String(item.description)}</span></li>)}</ol>{Array.isArray(pack.omitted) && pack.omitted.length > 0 && <details open><summary>省略 {pack.omitted.length} 张（生成前必须确认）</summary>{(pack.omitted as Array<Record<string, unknown>>).map((item) => <p key={String(item.id)}>{String(item.name || item.id)} · {String(item.reason)}</p>)}</details>}</div>}
    {references.length > 0 && <footer><button className="button button--primary" disabled={busy || !references.some((asset) => asset.approved)} onClick={() => run(() => copilotApi.approveArtifact(project.id, 'reference-inventory'), { label: '批准参考图清单' })}><Check size={15} />批准 Reference Inventory</button></footer>}
  </section>;
}
