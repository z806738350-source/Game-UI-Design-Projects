...
import { friendlyError, Dropdown } from '../shared/ui';
...
export function ScreenManager({ project, busy, onProject }: { project: DesignProject; busy: boolean; onProject: (project: DesignProject) => void }) {
  ...
  return <section className="screen-manager" data-testid="screen-manager"><div><span>ACTIVE SCREEN</span><b>{active?.name || project.screen_id}</b><small>{screens.length} 个页面 · {project.requirement ? '独立需求已配置' : '等待独立需求'}</small></div>{error && <span className="inline-error" role="alert" data-testid="screen-manager-error">{error}</span>}<Dropdown testId="screen-active-select" ariaLabel="切换当前页面" disabled={busy} value={project.screen_id} onChange={(next) => perform(() => copilotApi.setActiveScreen(project.id, next), false).then(() => copilotApi.openProject(project.id).then(onProject))} options={screens.map((screen) => ({ value: screen.id, label: screen.name }))} /><input value={rename} onChange={(event) => setRename(event.target.value)} aria-label="当前页面名称" placeholder="重命名页面" /><button disabled={busy || !rename.trim() || rename === active?.name} title="重命名当前页面" onClick={() => perform(() => copilotApi.updateScreen(project.id, project.screen_id, { name: rename }))}><Save size={14} /></button><input value={name} onChange={(event) => setName(event.target.value)} placeholder="新页面名称" /><button disabled={busy || !name.trim()} data-testid="screen-manager-create" title="创建独立页面" onClick={() => perform(() => copilotApi.createScreen(project.id, { name })).then(() => setName(''))}><Plus size={14} /></button><button disabled={busy} title="复制当前页面及全部产物" onClick={() => perform(() => copilotApi.duplicateScreen(project.id, project.screen_id))}><Copy size={14} /></button>{archiveCandidates.length > 0 && <><Dropdown testId="screen-archive-select" ariaLabel="选择要归档的页面" value={archiveTarget} onChange={(next) => setArchiveTarget(next)} options={archiveCandidates.map((screen) => ({ value: screen.id, label: screen.name }))} /><button disabled={busy || !archiveTarget} title="归档选中的非当前页面" onClick={() => perform(() => copilotApi.updateScreen(project.id, archiveTarget, { status: 'archived' }))}><Archive size={14} /></button></>}</section>;
}
