...
export function UnderlayWorkbench({ project }: { project: DesignProject }) {
  ...
  return <section className="evidence-workbench underlay-workbench" data-testid="underlay-workbench"><header><span>UNDERLAY WORKBENCH</span><b>结构约束、污染证据与修复链</b></header><div>{entries.map(({ label, artifact, badge }) => <article key={label} data-testid={`underlay-evidence-${label.toLowerCase()}`}><b>{label}</b>{artifact ? <><small className={String(artifact.status) === 'failed' ? 'is-failed' : ''}>{badge || `${String(artifact.status)} · V${String(artifact.version || 1)}`}</small><details><summary>查看证据 JSON</summary><pre>{JSON.stringify(label === 'Critique' ? { evidence: artifact.evidence, metrics: artifact.deterministic_metrics, issues: artifact.issues } : label === 'Repair' ? { attempt: artifact.attempt, mode: artifact.repair_mode, output: artifact.output, manual_review: artifact.manual_review } : { slots: artifact.slots, layout_guide: artifact.layout_guide }, null, 2)}</pre></details></> : <small>尚未生成</small>}</article>)}</div></section>;
}
